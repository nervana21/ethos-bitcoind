...
pub trait BitcoinClient: Send + Sync + TransportTrait + TransportExt + RpcDispatchExt {
    ...
    async fn get_addr_man_info(&self) -> Result<GetAddrManInfoResponse, Self::Error>;
    ...
    async fn get_raw_addr_man(&self) -> Result<GetRawAddrManResponse, Self::Error>;
    ...
    async fn get_tx_spending_prev_out(
        &self,
        outputs: Vec<serde_json::Value>,
    ...
    async fn load_tx_out_set(&self, path: String) -> Result<LoadTxOutSetResponse, Self::Error>;
    ...
    async fn set_tx_fee(&self, amount: serde_json::Value) -> Result<SetTxFeeResponse, Self::Error>;
...
impl<T: TransportTrait + TransportExt + Send + Sync> BitcoinClient for T {
    ...
    async fn set_tx_fee(&self, amount: serde_json::Value) -> Result<SetTxFeeResponse, Self::Error> {
        let mut rpc_params = vec![];
        rpc_params.push(serde_json::json!(amount));
        self.call::<SetTxFeeResponse>("settxfee", &rpc_params).await
    }
