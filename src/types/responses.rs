...
impl<'de> serde::Deserialize<'de> for GetBlockStatsResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        #[allow(clippy::needless_lifetimes)]
        impl<'de> Visitor<'de> for ConditionalResponseVisitor {
            type Value = GetBlockStatsResponse;
            ...
            {
                let mut avgfee = None;
                let mut avgfeerate = None;
                let mut avgtxsize = None;
                let mut blockhash = None;
                let mut feerate_percentiles = None;
                let mut height = None;
                let mut ins = None;
                let mut maxfee = None;
                let mut maxfeerate = None;
                let mut maxtxsize = None;
                let mut medianfee = None;
                let mut mediantime = None;
                let mut mediantxsize = None;
                let mut minfee = None;
                let mut minfeerate = None;
                let mut mintxsize = None;
                let mut outs = None;
                let mut subsidy = None;
                let mut swtotal_size = None;
                let mut swtotal_weight = None;
                let mut swtxs = None;
                let mut time = None;
                let mut total_out = None;
                let mut total_size = None;
                let mut total_weight = None;
                let mut totalfee = None;
                let mut txs = None;
                let mut utxo_increase = None;
                let mut utxo_increase_actual = None;
                let mut utxo_size_inc = None;
                let mut utxo_size_inc_actual = None;
                while let Some(key) = map.next_key::<String>()? {
                    if key == "avgfee" {
                        if avgfee.is_some() {
                            return Err(de::Error::duplicate_field("avgfee"));
                        }
                        avgfee = Some(map.next_value::<u64>()?);
                    }
                    if key == "avgfeerate" {
                        if avgfeerate.is_some() {
                            return Err(de::Error::duplicate_field("avgfeerate"));
                        }
                        avgfeerate = Some(map.next_value::<u64>()?);
                    }
                    if key == "avgtxsize" {
                        if avgtxsize.is_some() {
                            return Err(de::Error::duplicate_field("avgtxsize"));
                        }
                        avgtxsize = Some(map.next_value::<u64>()?);
                    }
                    if key == "blockhash" {
                    ...
                    if key == "feerate_percentiles" {
                        if feerate_percentiles.is_some() {
                            return Err(de::Error::duplicate_field("feerate_percentiles"));
                        }
                        feerate_percentiles = Some(map.next_value::<serde_json::Value>()?);
                    }
                    if key == "height" {
                        if height.is_some() {
                            return Err(de::Error::duplicate_field("height"));
                        }
                        height = Some(map.next_value::<u64>()?);
                    }
                    if key == "ins" {
                        if ins.is_some() {
                            return Err(de::Error::duplicate_field("ins"));
                        }
                        ins = Some(map.next_value::<u64>()?);
                    }
                    if key == "maxfee" {
                        if maxfee.is_some() {
                            return Err(de::Error::duplicate_field("maxfee"));
                        }
                        maxfee = Some(map.next_value::<u64>()?);
                    }
                    if key == "maxfeerate" {
                        if maxfeerate.is_some() {
                            return Err(de::Error::duplicate_field("maxfeerate"));
                        }
                        maxfeerate = Some(map.next_value::<f64>()?);
                    }
                    if key == "maxtxsize" {
                        if maxtxsize.is_some() {
                            return Err(de::Error::duplicate_field("maxtxsize"));
                        }
                        maxtxsize = Some(map.next_value::<u64>()?);
                    }
                    if key == "medianfee" {
                        if medianfee.is_some() {
                            return Err(de::Error::duplicate_field("medianfee"));
                        }
                        medianfee = Some(map.next_value::<u64>()?);
                    }
                    if key == "mediantime" {
                        if mediantime.is_some() {
                            return Err(de::Error::duplicate_field("mediantime"));
                        }
                        mediantime = Some(map.next_value::<u64>()?);
                    }
                    if key == "mediantxsize" {
                        if mediantxsize.is_some() {
                            return Err(de::Error::duplicate_field("mediantxsize"));
                        }
                        mediantxsize = Some(map.next_value::<u64>()?);
                    }
                    if key == "minfee" {
                        if minfee.is_some() {
                            return Err(de::Error::duplicate_field("minfee"));
                        }
                        minfee = Some(map.next_value::<u64>()?);
                    }
                    if key == "minfeerate" {
                        if minfeerate.is_some() {
                            return Err(de::Error::duplicate_field("minfeerate"));
                        }
                        minfeerate = Some(map.next_value::<u64>()?);
                    }
                    if key == "mintxsize" {
                        if mintxsize.is_some() {
                            return Err(de::Error::duplicate_field("mintxsize"));
                        }
                        mintxsize = Some(map.next_value::<u64>()?);
                    }
                    if key == "outs" {
                        if outs.is_some() {
                            return Err(de::Error::duplicate_field("outs"));
                        }
                        outs = Some(map.next_value::<u64>()?);
                    }
                    if key == "subsidy" {
                        if subsidy.is_some() {
                            return Err(de::Error::duplicate_field("subsidy"));
                        }
                        subsidy = Some(map.next_value::<u64>()?);
                    }
                    if key == "swtotal_size" {
                        if swtotal_size.is_some() {
                            return Err(de::Error::duplicate_field("swtotal_size"));
                        }
                        swtotal_size = Some(map.next_value::<u64>()?);
                    }
                    if key == "swtotal_weight" {
                        if swtotal_weight.is_some() {
                            return Err(de::Error::duplicate_field("swtotal_weight"));
                        }
                        swtotal_weight = Some(map.next_value::<u64>()?);
                    }
                    if key == "swtxs" {
                        if swtxs.is_some() {
                            return Err(de::Error::duplicate_field("swtxs"));
                        }
                        swtxs = Some(map.next_value::<u64>()?);
                    }
                    if key == "time" {
                        if time.is_some() {
                    ...
                    if key == "total_out" {
                        if total_out.is_some() {
                            return Err(de::Error::duplicate_field("total_out"));
                        }
                        total_out = Some(map.next_value::<u64>()?);
                    }
                    if key == "total_size" {
                        if total_size.is_some() {
                            return Err(de::Error::duplicate_field("total_size"));
                        }
                        total_size = Some(map.next_value::<u64>()?);
                    }
                    if key == "total_weight" {
                        if total_weight.is_some() {
                            return Err(de::Error::duplicate_field("total_weight"));
                        }
                        total_weight = Some(map.next_value::<u64>()?);
                    }
                    if key == "totalfee" {
                        if totalfee.is_some() {
                            return Err(de::Error::duplicate_field("totalfee"));
                        }
                        totalfee = Some(map.next_value::<u64>()?);
                    }
                    if key == "txs" {
                        if txs.is_some() {
                            return Err(de::Error::duplicate_field("txs"));
                        }
                        txs = Some(map.next_value::<u64>()?);
                    }
                    if key == "utxo_increase" {
                        if utxo_increase.is_some() {
                            return Err(de::Error::duplicate_field("utxo_increase"));
                        }
                        utxo_increase = Some(map.next_value::<u64>()?);
                    }
                    if key == "utxo_increase_actual" {
                        if utxo_increase_actual.is_some() {
                            return Err(de::Error::duplicate_field("utxo_increase_actual"));
                        }
                        utxo_increase_actual = Some(map.next_value::<u64>()?);
                    }
                    if key == "utxo_size_inc" {
                        if utxo_size_inc.is_some() {
                            return Err(de::Error::duplicate_field("utxo_size_inc"));
                        }
                        utxo_size_inc = Some(map.next_value::<u64>()?);
                    }
                    if key == "utxo_size_inc_actual" {
                        if utxo_size_inc_actual.is_some() {
                            return Err(de::Error::duplicate_field("utxo_size_inc_actual"));
                        }
                        utxo_size_inc_actual = Some(map.next_value::<u64>()?);
                    } else {
                        let _ = map.next_value::<de::IgnoredAny>()?;
                    }
                }
