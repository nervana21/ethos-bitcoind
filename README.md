...
A type-safe Rust client for Bitcoin Core v32.0 RPCs.
...
## Consumer aliases (Floresta and friends)

Short names for the light-client RPC surface live in `rpc_prelude` / `aliases` and are re-exported at the crate root:

```rust
use ethos_bitcoind::{GetBlockVerboseOne, GetTxOut, ScriptPubKey, GetBlockchainInfo};
// or: use ethos_bitcoind::aliases::*;
```
